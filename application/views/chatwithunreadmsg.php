...
<html class=" ">
    <head>
		...
		<style>
			...
			.noMoreMessage{color: #ef7474;background: #fde9e9;padding: 5px;text-align: center;width: 250px;margin-left: -15px;margin-top: -15px;}
			.blink_me {background:#565090;}
			.unreadAlert {animation: blinker2 1s linear infinite; padding: 5px;text-align: center;font-weight: bold;font-family: monospace;border-bottom: 1px dotted #ccc;color:#bb1717;}
			...
			#controlbar-alert{background:none;}
			@keyframes blinker {  
			  100% { padding:2px; }
			}
			@keyframes blinker2 {  
                     ...
                     <ul class="info-menu right-links list-inline list-unstyled">
                         ...
                         <li class="chat-toggle-wrapper">
                             <a href="#" data-toggle="chatbar" class="toggle_chat">
                                 ...
                                 <span class="badge badge-accent" id="unreadChat" style="background:#ff5f74;"><?php echo $this->db->where('chat_status','unseen')->where('chat_empid',$this->session->userdata('id'))->count_all_results('chating');?></span>
		 ...
		 <script>
			...
			function actionRequest(id, status){
				...
				$.ajax({
					...
					dataType:'JSON',
					success:function(data){
						//console.log('first='+u_id);
						//$('#request_'+id).fadeOut(1000);
						if(status == 'accept'){
					...
					if ($(this).hasClass("active")) {
						...
						if ($(".chatapi-windows #user-window" + id).length) {
							...
							$(".chatapi-windows #user-window" + id).removeClass("minimizeit").show();
								...
								$.ajax({
									type:'POST',
									url:'<?php echo base_url('Admin/getChating'); ?>',
									data:{employee_id:id},
									dataType:'JSON',
									success:function(data){
										...
										if(data){
											...
											if(data.seen != ''){
												items_unseen = "";
												$.each(data.seen, function(index) {
													lastId = data.seen[0]["chat_id"];
													var user_name = "";
													var backcolor;
													data.seen[index]['chat_userid'] == <?php echo $this->session->userdata('id'); ?> ? user_name = "You"  : user_name =getUserName(data.seen[index]['chat_userid']);
													data.seen[index]['chat_userid'] == <?php echo $this->session->userdata('id'); ?> ?  style = 'background:#e6e9ed;color:#333;' : style ='background:#565090;color:#fff;';
													items = items + "<div class='chatmsg msg_save'><span class='name'>"+user_name+"</span><span class='text' style='"+style+"'>"+data.seen[index]['chat_msg']+"</span><span class='ts'>"+timeDifference(data.seen[index]['chat_datetime'])+"</span></div>";
												});
											}
											if(data.unseen != ''){
												//var counter = count(data.unseen);
												var count = data.unseen.length;
												var unreadAlert = "<p class='unreadAlert'>"+count+" unread messages</p>";
												$.each(data.unseen, function(index) {
													//lastId = data.unseen[0]["chat_id"];
													var user_name = "";
													var backcolor;
													data.unseen[index]['chat_userid'] == <?php echo $this->session->userdata('id'); ?> ? user_name = "You"  : user_name =getUserName(data.unseen[index]['chat_userid']);
													data.unseen[index]['chat_userid'] == <?php echo $this->session->userdata('id'); ?> ?  style = 'background:#e6e9ed;color:#333;' : style ='background:#565090;color:#fff;';
													items_unseen = items_unseen + "<div class='chatmsg msg_save'><span class='name'>"+user_name+"</span><span class='text' style='"+style+"'>"+data.unseen[index]['chat_msg']+"</span><span class='ts'>"+timeDifference(data.unseen[index]['chat_datetime'])+"</span></div>";
												});
												items_unseen = unreadAlert+items_unseen;
											}
											...
											html += "<img id='loader' src='http://opengraphicdesign.com/wp-content/uploads/2009/01/loader64.gif'><div class='chatarea'>"+items+items_unseen+"</div>";
											...
											/// DATA LOAD ON SCROLL UP
											$(document).ready(function(){           
												$(".chatapi-windows #user-window"+id+" .chatarea").scrollTop($(".chatapi-windows #user-window"+id+" .chatarea")[0].scrollHeight);
												$(".chatapi-windows #user-window"+id+" .chatarea").scroll(function(){
													if ($(".chatapi-windows #user-window"+id+" .chatarea").scrollTop() == 0){
														$('#loader').show();
														$.ajax({
															type:'POST',
															url:'<?php echo base_url('Admin/loadConversation'); ?>',
															data: {employee_id:id,lastId : lastId}, // This line shows sending the data.  How you get it is up to you
															dataType:'JSON',
															success:function(data){
																if(!noMoreMessage){
																	var items = "";
																	...
																	//console.log(prepend_items);
																	$("#user-window"+id+" .chatarea").prepend(items);
																	$(".noMoreMessage").toggleClass('fadeInDown');
																	$('#loader').hide();
																	//$("#user-window"+id+" .chatarea").scrollTop(50);
																}else{
																	$('#loader').hide();
																	//$("#user-window"+id+" .chatarea").scrollTop(50);
																}
